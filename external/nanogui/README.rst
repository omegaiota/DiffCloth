...
.. code-block:: cpp
   ...
   FormHelper *SettingsControl = new FormHelper(screen);
   ref<Window> window = SettingsControl->addWindow(Eigen::Vector2i(10, 10), "Form helper example");
   SettingsControl->addGroup("Basic types");
   SettingsControl->addVariable("bool", bvar);
   SettingsControl->addVariable("string", strvar);

   SettingsControl->addGroup("Validating fields");
   SettingsControl->addVariable("int", ivar);
   SettingsControl->addVariable("float", fvar);
   SettingsControl->addVariable("double", dvar);

   SettingsControl->addGroup("Complex types");
   SettingsControl->addVariable("Enumeration", enumval, enabled)
      ->setItems({"Item 1", "Item 2", "Item 3"});
   SettingsControl->addVariable("Color", colval);

   SettingsControl->addGroup("Other widgets");
   SettingsControl->addButton("A button", [](){ std::cout << "Button pressed." << std::endl; });
