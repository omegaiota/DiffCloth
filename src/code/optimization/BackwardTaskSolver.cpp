...
void BackwardTaskSolver::optimizeLBFGS(Simulation *system, OptimizeHelper &helper, int FORWARD_STEPS, int demoNum,
    bool isRandom, int srandSeed, const std::function<void(const std::string &)> &setTextBoxCB) {
  ...
  double minLoss = 0;
  try {
    int niter = solver.minimize(helper, bestParam, minLoss, helper.getLowerBound(), helper.getUpperBound());
    system->exportStatistics((Demos)demoNum, helper.statistics, helper.taskInfo, true);
    if (niter == 1) helper.saveLastIter();
...
OptimizeHelper *BackwardTaskSolver::getOptimizeHelperPointer(Simulation *system, int demoNum) {
  ...
  return new OptimizeHelper(static_cast<Demos>(demoNum), helper.system, helper.lossInfo, helper.taskInfo,
      helper.lossType, helper.system->sceneConfig.stepNum, helper.param_actual);
}
...
OptimizeHelper BackwardTaskSolver::getOptimizeHelper(Simulation *system, int demoNum) {
  ...
  return OptimizeHelper(
      static_cast<Demos>(demoNum), system, lossInfo, taskInfo, lossType, system->sceneConfig.stepNum, paramGroundtruth);
}
