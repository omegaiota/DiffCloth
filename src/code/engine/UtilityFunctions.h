...
static void insertIntoTriplets(
    TripleVector& triplets, Eigen::Matrix<double, rows, cols>& src, int startRow, int startCol) {
...
template <int rows, int cols>
static void insertIntoTriplets(TripleVector& triplets, Eigen::Matrix<double, rows, cols>& src, int blockRows,
    int blockCols, int srcStartRows, int srcStartCols, int dstStartRow, int dstStartCol) {
...
static Eigen::Matrix<double, m * p, n * q> kronecker(
    const Eigen::Matrix<double, m, n>& A, const Eigen::Matrix<double, p, q>& B) {
...
static std::vector<std::string> listDirectory(std::string path) {
  ...
  if (dirFile) {
    ...
    while ((hFile = readdir(dirFile)) != NULL) {
      ...
      if (hFile->d_type == DT_DIR) filePaths.emplace_back(hFile->d_name);
    }
    closedir(dirFile);
  }
...
static std::string vec2str(VecXd v, int precision = 2) {
  ...
  for (int i = 1; i < v.rows(); i++) streamObj3 << "," << v[i];
...
static std::string vec2str(Eigen::Matrix<double, m, 1> v, int precision = 2) {
  ...
  for (int i = 1; i < m; i++) streamObj3 << "," << v[i];
