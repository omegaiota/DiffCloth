...
public:
  ...
  Camera()
      : lookAtPos(0, 0, 0), camPos(glm::vec3(-22.14, 9.24, 7.59)), speed(0.05f),
  ...
  void translateCamPos(glm::vec3 pos) { setCamPos(camPos + pos); }
  ...
  void moveZ(float time) { translateCamPos(time * glm::vec3(0, 0, 1)); }

  void moveNegZ(float time) { moveZ(-1 * time); }
  ...
  void moveX(float time) { translateCamPos(time * glm::vec3(1, 0, 0)); }

  void zoom(float percentage) {
    glm::vec3 newPos = lookAtPos * (percentage / 100.0f) +
                       camPos * (1.0f - percentage / 100.0f);
    setCamPos(newPos);
  }

  void moveNegX(float time) { moveX(-1 * time); }

  void moveY(float time) { translateCamPos(time * glm::vec3(0, 1, 0)); }

  void moveNegY(float time) { moveY(-1 * time); }

  void setFoVDeg(float deg) { FOVdegree = deg; }
  ...
  void addPitchAndYaw(float p, float y) { setPitchAndYaw(pitch + p, yaw + y); }

  glm::vec3 getCamRight() { return normalize(cross(camFront, camUp)); }
...
private:
  ...
  void updateViewMat() { viewMat = lookAt(camPos, lookAtPos, camUp); }
};
