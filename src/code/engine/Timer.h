...
public:
    bool enabled;
    int numCounters;
    std::chrono::steady_clock::time_point lastTimeStep, start, end, debugTimeStamp;
    ...
    static std::vector<std::pair<std::string,long long>> addTimer( std::vector<std::pair<std::string,long long>> a,  std::vector<std::pair<std::string,long long>> b) {
      if (a.size() != b.size()) {
//        std::printf("ERROR: timer cannot be add up: size %zu %zu is different. return a\n", a.size(), b.size());
//        std::printf("labels in a:");
//        for (std::pair<std::string, long long>& item : a)
...
//          std::printf("ERROR: timer cannot be add up: label %s and %s are different. return a\n", itemA.first.c_str(), itemB.first.c_str());
        }
    ...
    long long getTimeMicroSeconds(std::chrono::steady_clock::time_point& begin, std::chrono::steady_clock::time_point& end) {
      long long duration = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
      return duration;
    }

    long long getTimeMilliSeconds(std::chrono::steady_clock::time_point& begin, std::chrono::steady_clock::time_point& end) {
...
//      std::printf("Toc at %s iter totalAccum %lld startEnd %lld error: %.7f\n", lastLabel.c_str(), totalTime,startEnd, std::abs(totalTime - startEnd) * 1.0 / startEnd);
    ...
    void report() {
      ...
      for (int i = 0; i < durationsMicroseconds.size(); i++) {
        ...
        std::printf("%s:\t%lld[µs]\t% .2f%%\n", labels[i].c_str(), duration, (duration * 1.0 / durationTotal) * 100.0);
      }
...
//        std::printf("ERROR: timer duration size %zu is not equal to label size %zu\n", durationsMicroseconds.size(), labels.size());
        return ret;
      }
      long long totalTime = 0;
      for (int i = 0; i < num; i++) {
    ...
    PerformanceTiming getSolveTimingReport() {
      perf.nonSolveTimeMicroseconds =
              getTimeMicroSeconds(start, end) - perf.solveDirectMicroseconds - perf.solveIterativeMicroseconds;
      return perf;
    };
