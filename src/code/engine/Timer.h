...
public:
  ...
  static std::vector<std::pair<std::string, long long>> addTimer(
      std::vector<std::pair<std::string, long long>> a, std::vector<std::pair<std::string, long long>> b) {
    if (a.size() != b.size()) {
...
public:
  ...
  int toc() {
    ...
    //      totalTime,startEnd, std::abs(totalTime - startEnd) * 1.0 / startEnd);
  ...
  void report() {
    ...
    for (long long d : durationsMicroseconds) durationTotal += d;
    for (int i = 0; i < durationsMicroseconds.size(); i++) {
