...
public:
    ...
    Spline(Vec3d p0, Vec3d p1, double yUp, int pFixed, double startFraction = 0.0, double endFraction = 1.0) : pFixed(pFixed) {
      Segment seg;
      seg.p0 = p0;
      seg.p1 = p1;
      seg.yUp = yUp;
    ...
    static double p0Constant(double t, int order = 0) { // t should be the actual spline percentage, so t=0 will map to splineStart and t=1 will map to splineEnd
      double t_3 = t * t * t;
      double t_2 = t * t;
