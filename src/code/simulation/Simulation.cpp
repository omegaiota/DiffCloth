		...
		if (selfcollisionEnabled) {
			...
			for (std::vector<SelfCollisionInformation> &selfInfos :
				...
				for (SelfCollisionInformation &info : selfInfos) {
					...
					Vec3d r_i =
							k * calcualteDryFrictionForce(info.normal, info.d, clothFrictionalCoeff, info.type);
					info.r = r_i;
...
void Simulation::loadWindSim2RealAnimationSequence(
	...
	sceneConfig.camFocusPointType = CameraFocusPointType::CAMERA_POINT;
...
void Simulation::exportCurrentSimulation(std::string fullFileName) {
	...
	for (int i = 0; i < forwardRecords.size(); i++) {
		...
		MeshFileHandler::saveOBJFile(
				subFolderPrefix + "/" + "0-CLOTH", forwardRecords[i].x,
		...
		for (int primId = 0; primId < primitives.size(); primId++) {
			...
			MeshFileHandler::saveOBJFile(
					subFolderPrefix + "/" + std::to_string(primId + 1) + "-" + name,
					primPos, getParticleNormals(primMesh, primPos), primMesh);
		}
		...
		for (int fixedPointId = 0;
			...
			MeshFileHandler::saveOBJFile(
					subFolderPrefix + "/" +
							std::to_string(primitives.size() + fixedPointId + 1) + "-" +
							"CLIP",
					primPos, primMesh);
		}
		exportFrameInfo(this, forwardRecords[i],
				subFolderPrefix + "/" + "info.txt");
	}
...
void Simulation::exportStatistics(Demos demoIdx,
	...
	for (int i = statistics.optimizationRecordsSaved; i < currentRecordSize;
			i++) {
		std::string iterationFolder = OUTPUT_PARENT_FOLDER +
				statistics.experimentName + subFolder +
	...
	// Last frame meshes
	std::string lastFrameMeshFolder = OUTPUT_PARENT_FOLDER +
			statistics.experimentName + subFolder +
			"last_frame_meshes/";
...
void Simulation::exportStatsSimulations(
	...
	for (int i = prevWrriten; i < newWritten; i++) {
		...
		exportFrameInfo(this, records[i].second, subFolder + "/info.txt");
	}
}
