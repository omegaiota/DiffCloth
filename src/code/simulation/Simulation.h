...
public:
  ...
  BackwardInformation backwardInfoDefault = {.dL_dx = VecXd(0),
      .dL_dv = VecXd(0),
  ...
  struct TaskSolveStatistics {
    ...
    std::vector<std::pair<ParamInfo, ForwardInformation>>
        completeForwardLog;  // For each simulation, only log the last frame of forward sim
    std::vector<std::pair<ParamInfo, BackwardInformation>>
        completeBackwardLog;  // For each backprop, only log the first frame of backprop
  ...
  struct SceneConfiguration {
    ...
    std::vector<std::pair<double, std::vector<int>>>
        customAttachmentVertexIdx;  // outside loop: different sets; each element: startFrame x vIdx
  ...
  struct FileMesh {
    ...
    void getMesh(std::vector<Vec3i> &mesh, VecXd &primPos, Vec3d center) {
      ...
      for (Vec3i &t : triangles) mesh.emplace_back(t);
  ...
  struct SystemMatrix {  // All these matrices are constraint weight dependent, so if constraint weight change, these
                         // matrices change
    // Additionally, in our simulation sometimes the constraints need to be changed because of change of attachment
    // constraints
    SpMat P;
    SpMat A;
    SpMat A_t;
  ...
  VecXd x_n, v_n, gravity_n,
      external_force_field;  // velocity and position from previous solved state / beginning of current timestep
  ...
  static Simulation *createSystem(Simulation::SceneConfiguration sceneConfig, Vec3d center, bool runBackward = true);
  ...
  void setWindAncCollision(
      bool windEnable, bool collisionEnable, bool selfCollisionEnable, bool enableConstantForcefield = false) {
  ...
  BackwardInformation stepBackwardNN(Simulation::BackwardTaskInformation &taskInfo, VecXd &dL_dxnew, VecXd &dL_dvnew,
      const ForwardInformation &forwardInfo_new, bool isStart, const VecXd &dL_dxinit, const VecXd &dL_dvinit);

  BackwardInformation stepBackward(Simulation::BackwardTaskInformation &taskInfo, BackwardInformation &gradient_new,
      const ForwardInformation &forwardInfo_new, bool isStart, const VecXd &dL_dxinit, const VecXd &dL_dvinit);
  ...
  void resetForwardRecordsFromFolder(std::string subFolder) {
    ...
    std::sort(frameFileNames.begin(), frameFileNames.end(), [](const std::string a, const std::string b) -> bool {
    ...
    for (int i = 0; i < frameFileNames.size(); i++) {
      ...
      for (Primitive *p : primitives) {
        ...
        if (p->isPrimitiveCollection) {  // TODO: this is a hacky way to reset primitive trees with <= 2 levels; should
                                         // be changed to recursion
  ...
  void rotatePointsAccordingToConfig(std::vector<Vec3d> &particleIn) {
    switch (sceneConfig.orientation) {
      ...
      case Orientation::BACK: {
        std::printf("orientation down: rotate\n");
        particleIn = rotatePointsAroundCenter(particleIn,
            axisToRotation(Vec3d(0, 0, 1), Vec3d(1, 0, 0)) * axisToRotation(Vec3d(1, 0, 0), Vec3d(0, 0, -1)));
        break;
      }
  ...
  void loadWindSim2RealAnimationSequence(
      std::string folderName, std::vector<std::string> files, bool isRyanWhite = true);

  void initScene();

  SelfCollisionInformation isSelfCollision(const Particle &a, const Particle &b, const Vec3d &x_a, const Vec3d &x_b,
      const Vec3d &v_a, const Vec3d &v_b) const;

  Simulation::SelfCollisionInformation isSelfCollision(
      const Triangle &a, const Particle &p3, const Vec3d v_a, const Vec3d v_b) const;

  PrimitiveCollisionInformation isInContactWithObstacle(
  ...
  double calculateLossAndGradient(
      LossType &lossType, LossInfo &lossInfo, VecXd &dL_dx, VecXd &dL_dv, int idx, bool calculateLoss);

  void resetSystemWithParams(BackwardTaskInformation &taskConfiguration, ParamInfo &param);

  void calculateFiniteDiffLossArr(
      ParamInfo paramPlus, ParamInfo paramMinus, double delta, std::vector<ForwardInformation> &centerForward,
      std::vector<double> &L_arr, BackwardTaskInformation taskConfiguration, LossType lossType, LossInfo &lossInfo,
      int FORWARD_STEPS,
  ...
  std::vector<BackwardInformation> finiteDifferenceBackward(
      BackwardTaskInformation taskConfiguration, LossType lossType, LossInfo &lossInfo, int FORWARD_STEPS,
      ParamInfo guess, const std::function<void(const std::string &value)> &setTextBoxCB = [](const std::string &v) {});

  static std::string forwardInfoToString(
      BackwardTaskInformation taskInfo, const Simulation::ForwardInformation &forwardInfo) {
    std::string out;
    out += "============Forward Info:======================\n";
  ...
  static std::string backwrdInfoAndGradToString(
      const Simulation::BackwardTaskInformation &taskInfo, BackwardInformation grad);
  ...
  void exportStatistics(
      Demos demoIdx, TaskSolveStatistics &statistics, BackwardTaskInformation taskInfo, bool writePerf = false);
  ...
  void exportSimulation(
      std::string fileName, std::vector<ForwardInformation> &records, bool staticPrimitivesFirstFrameOnly = true);
  ...
  std::vector<BackwardInformation> runBackwardTask(
      ...
      TaskSolveStatistics &taskStatistics, int FORWARD_STEPS, ParamInfo guess, bool lossOnly,
      const std::function<void(const std::string &value)> &setTextBoxCB = [](const std::string &v) {},
  ...
  std::pair<collisionInfoPair, std::vector<std::vector<SelfCollisionInformation>>> collisionDetection(
      const VecXd &x_n, const VecXd &v, const VecXd &x_prim, const VecXd &v_prim);

  std::vector<std::vector<Simulation::SelfCollisionInformation>> contactSorting(
  ...
  std::pair<VecXd, VecXd> calculateDryFrictionVector(
      const VecXd &f, std::pair<collisionInfoPair, std::vector<std::vector<SelfCollisionInformation>>> &detectionInfos);

  std::pair<std::pair<SpMat, SpMat>, VecXd> calculatedr_df(
      const completeCollisionInfo &infos, bool calculatePrimitiveGradient, bool calculateDensityGradient) const;

  VecXd calculatedr_ddensity(const completeCollisionInfo &detectionInfos, bool calculatePrimitiveGradient) const;

  std::vector<VecXd> calculatedr_dmu(
  ...
  void resetSystem(std::vector<std::pair<int, double>> &primitiveMus) { resetSystem(); }
...
private:
  ...
  static SpMat factorizeDirectSolverLLT(
      const SpMat &A, Eigen::SimplicialLLT<SpMat> &lltSolver, const std::string &warning_msg);

  SpMat factorizeDirectSolverSparseLU(
      const SpMat &A, Eigen::SparseLU<SpMat> &lltSolver, const std::string &warning_msg);

  std::chrono::steady_clock::time_point getTimeNow() { return std::chrono::steady_clock::now(); }
...
public:
  ...
  ForwardInformation getStateInfo() { return forwardRecords[forwardRecords.size() - 1]; }

  // get the forward info of the current step
  ForwardInformation getPastStateInfo(int stepIdx) { return forwardRecords[stepIdx]; }
  ...
  VecXd solveDirect(VecXd &dL_dxnew, double t_2, SpMat &dproj_dxnew_t, SystemMatrix &currentSysMat,
