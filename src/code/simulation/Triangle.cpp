...
double Triangle::evaluateEnergy(const Mat3x2d &F, const VecXd &x_new) {
  switch (energyType) {
  case QUADRATIC:
    energyBuffer = 0.5 * k_stiff * area_rest *
                   (F - projectToManifold(x_new)).squaredNorm();
    return energyBuffer;
    break;
  default:
  case NON_QUADRATIC:
    ...
    double m_energy =
        area_rest *
        (k[0] * G1(0, 0) * G1(0, 0) + k[2] * G1(1, 1) * G1(1, 1) +
         k[1] * G1(0, 0) * G1(1, 1) * 2.0f + k[3] * G1(0, 1) * G1(0, 1)) /
        2.0f;
...
Vec9d Triangle::stretchingForce(const VecXd &x_new) {
  ...
  case QUADRATIC: {
    ...
    stretchingForceBuffer =
        -k_stiff * area_rest * P_vec.transpose() *
        (dF_dx.transpose() - dproj_dx.transpose()).transpose();
...
Vec9d Triangle::dfi_dk(const VecXd &x_new) {
  ...
  case QUADRATIC: {
    ...
    dfi_dk_buffer =
        stretchingForce(x_new) / (constrainWeightSqrt * constrainWeightSqrt);
...
Triangle::stretchingForceAndJacobianCheck() const {
  ...
  Mat9x9d hess_e2 =
      k[0] * (outer(fuu_2, fuu_2) +
              std::fmax(e_uu, 0.) * Du.transpose() * Du) // Jacobian of force
      +
...
Mat3x2d Triangle::projectToManifold(const VecXd &x_vec) const {
  ...
  newdeltaUV.col(1) =
      (p.col(1) - p.col(1).dot(newdeltaUV.col(0)) * newdeltaUV.col(0))
          .normalized();

  Mat2x2d F_2D = newdeltaUV.transpose() * F;
  Eigen::JacobiSVD<Mat2x2d> svd(F_2D,
...
Mat6x9d Triangle::projectToManifoldBackward(const VecXd &x_vec) const {
  ...
  Mat3x3d df_daNorm =
      -(aNorm * b.transpose() + aNorm.transpose() * b * I_three);
  ...
  // backward R--> F
  Mat2x2d S = svd.matrixV() * svd.singularValues().asDiagonal() *
...
Triangle::Triangle(int p0_idx, int p1_idx, int p2_idx,
                   std::vector<Particle> &pArr, bool checkArea)
    : Constraint(Constraint::ConstraintType::CONSTRAINT_TRIANGLE, 6),
      p0_idx(p0_idx), p1_idx(p1_idx), p2_idx(p2_idx), pArr(pArr),
      color(0, 0, 0), overrideColor(false), energyType(QUADRATIC) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    double angle =
        getAngleABCInDegreeBetween(pArr[id1].pos_rest, pArr[id2].pos_rest,
                                   pArr[id3].pos_rest) *
        180.0 / 3.14159265;
    if (angle < 20) {
      std::printf("WARNING: MESH TOPOLOGY: angle between point %d-%d-%d is "
                  "%.2f degree\n",
                  id1, id2, id3, angle);
    }
  }
  ...
  if (checkArea) {
    if (area_rest < 0.001) {
      std::printf(
          "WARNING: MESH TOPOLOGY: Triangle (%d, %d, %d) has area %.4f\n",
