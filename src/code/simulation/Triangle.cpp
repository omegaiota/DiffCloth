  ...
  switch (energyType) {
    ...
    case NON_QUADRATIC:
      ...
      double m_energy = area_rest *
                        (k[0] * G1(0, 0) * G1(0, 0) + k[2] * G1(1, 1) * G1(1, 1) + k[1] * G1(0, 0) * G1(1, 1) * 2.0f +
                            k[3] * G1(0, 1) * G1(0, 1)) /
                        2.0f;
...
double Triangle::evaluateEnergy(const VecXd &x_new) { return evaluateEnergy(getDeformationGradient(x_new), x_new); }
...
std::pair<std::vector<Vec9d>, Mat9x9d> Triangle::stretchingForceAndJacobianCheck() const {
  ...
  Eigen::Matrix<double, 3, 9> Du = kronecker<1, 3, 3, 3>(du, I_three), Dv = kronecker<1, 3, 3, 3>(dv, I_three);
  ...
  Vec9d grad_e2 = (k[0] * e_uu + k[1] * e_vv) * fuu_2 + (k[2] * e_vv + k[1] * e_uu) * fvv_2  //  force
                  + 2 * k[3] * e_uv * fuv_2;
  ...
  Mat9x9d hess_e2 = k[0] * (outer(fuu_2, fuu_2) + std::fmax(e_uu, 0.) * Du.transpose() * Du)  // Jacobian of force
                    ...
                    k[1] * (outer(fuu_2, fvv_2) + std::fmax(e_uu, 0.) * Dv.transpose() * Dv + outer(fvv_2, fuu_2) +
                               std::fmax(e_vv, 0.) * Du.transpose() * Du) +
  ...
  switch (energyType) {
    ...
    case NON_QUADRATIC: {
      ...
      Mat9x9d hess_e = k[0] * (outer(fuu, fuu) + std::fmax(e_uu, 0.) * Du.transpose() * Du)  // Jacobian of force
                       ...
                       k[1] * ((outer(fuu, fvv)) + std::fmax(e_uu, 0.) * Dv.transpose() * Dv + (outer(fvv, fuu)) +
                                  std::fmax(e_vv, 0.) * Du.transpose() * Du) +
                       2. * k[3] * (outer(fuv, fuv));
...
void Triangle::addConstraint(std::vector<Triplet> &tri, int &c_idx, bool withWeight) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int dim = 0; dim < 3; dim++) {
      tri.emplace_back(c_idx + dim + 3 * i, p0()->idx * 3 + dim, -weightUsed * (inv_deltaUV(0, i) + inv_deltaUV(1, i)));
...
Triangle::Triangle(int p0_idx, int p1_idx, int p2_idx, std::vector<Particle> &pArr, bool checkArea)
  ...
  for (int i = 0; i < 3; i++) {
    ...
    double angle =
        getAngleABCInDegreeBetween(pArr[id1].pos_rest, pArr[id2].pos_rest, pArr[id3].pos_rest) * 180.0 / 3.14159265;
    if (angle < 20) {
