...
public:
  static void runExample() {
     ...
     * @param FabricConfiguration: configs for the fabric, including the mesh
     * model's path, color, dimensions, initial pos files etc
     ...
     *
     * After creating the system, this config will be stored in
     * Simulation::sceneConfig
     *
     * @param runBackward needs be set to true if you intend to run the backward
     * pass.
     */
    ...
    clothSystem->sceneConfig.trajectory =
    ...
    std::cerr << clothSystem
                     ->forwardRecords[clothSystem->forwardRecords.size() - 1]
                     .x.transpose()
              << std::endl;

    // All ran simulation are stored in Simulation::forwardRecords. Each record
    // stores the complete state detail of the simulation
    ...
    int fixedPointNumber =
        clothSystem->sysMat[clothSystem->currentSysmatId].fixedPoints.size();
    ...
    RenderLoop::renderRecordsForSystem(
        clothSystem, clothSystem->forwardRecords, true,
        "Visualization for pySimulations::runExample");

    // After simulation, each step pushes a record to
    std::printf("After forward simulation with %d steps, forwardRecords.size() "
                "is %zu\n",
                N, clothSystem->forwardRecords.size());

    //        for (int i = 0; i < N; i += 15)
    //            clothSystem->exportCurrentMeshPos(i, "wearhat-" +
    //            std::to_string(i));
    ...
    // Run backward pass. The backward pass resets the system, run forward pass
    // with the initialParameters, calculate its loss, and calclate the
    // gradients. Gradients are returned as a vector of BackwardInformation
    // forwardRecords[i] steps the system from [x,v]_{i-1} --> [x,v]_{i}, while
    // backwardRecords[i] backprops the system from [x,v]_{i} --> [x,v]_{i-1}
    //      std::vector<Simulation::BackwardInformation> backwardRecords =
    //      clothSystem->runBackwardTask(taskInfo,
    //                                                                                                  lossType,
    //                                                                                                  lossInfo,
    //                                                                                                  N,
    //                                                                                                  initialParameters,
  ...
  static void runExample2() {
    ...
    OptimizeHelper helper = BackwardTaskSolver::getOptimizeHelper(
        clothSystem, Demos::DEMO_WEAR_SOCK);
    //      OptimizeHelper helper =
    //      BackwardTaskSolver::getOptimizeHelper(clothSystem,
    //      Demos::DEMO_WEAR_HAT);

    for (int i = 0; i < 3; i++) {
      VecXd randomInitPrams = helper.getRandomParam(time(NULL));
      std::cout << "random parameters:" << randomInitPrams.transpose() << "\n";
      Simulation::ParamInfo paramInfo =
          helper.vecXdToParamInfo(randomInitPrams);
      std::printf(
          "Corresponds to info: %s\n",
      ...
      RenderLoop::renderRecordsForSystem(
          clothSystem, clothSystem->forwardRecords, false, false,
          "Set text here for whatever you need (only single line is "
          "supported): Visualization for pySimulations::runExample");
    }
  }
  ...
  static void runSec6_1_SydId_Wind_Tshirt() {
    ...
    Simulation::forwardConvergenceThreshold = 1e-5;
    OptimizeHelper helper = BackwardTaskSolver::getOptimizeHelper(
        clothSystem, Demos::DEMO_WIND_TSHIRT);
    //      OptimizeHelper helper =
    //      BackwardTaskSolver::getOptimizeHelper(clothSystem,
    //      Demos::DEMO_WEAR_HAT);
  ...
  static void runSec6_3_inverseDesign() {
    ...
    OptimizeHelper helper = BackwardTaskSolver::getOptimizeHelper(
        clothSystem, Demos::DEMO_DRESS_TWIRL);
    //      OptimizeHelper helper =
    //      BackwardTaskSolver::getOptimizeHelper(clothSystem,
    //      Demos::DEMO_WEAR_HAT);
  ...
  static void runSec6_4_sim2real() {
    ...
    Simulation::forwardConvergenceThreshold = 1e-9;
    OptimizeHelper helper = BackwardTaskSolver::getOptimizeHelper(
        clothSystem, Demos::DEMO_WIND_SIM2REAL);
    //      OptimizeHelper helper =
    //      BackwardTaskSolver::getOptimizeHelper(clothSystem,
    //      Demos::DEMO_WEAR_HAT);
  ...
  static void runSec6_1_sphere() {
    ...
    Simulation::forwardConvergenceThreshold = 1e-9;
    OptimizeHelper helper = BackwardTaskSolver::getOptimizeHelper(
        clothSystem, Demos::DEMO_SPHERE_ROTATE);
