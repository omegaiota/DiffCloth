...
public:
  enum EnergyType { QUADRATIC, NON_QUADRATIC };
  ...
  void hessianPrecalc(const VecXd &x_new) { hessian_buffer = stretchingHessian(x_new); }
  ...
  Vec9d dDeformation_dxnew(const VecXd &x) const { return dArea_dxnew(x) / area_rest; }

  Mat3x3d skew(const Vec3d &a) const {  // a x b = skew(a) x b
    std::vector<double> skewElements = {0, -a[2], a[1], a[2], 0, -a[0], -a[1], a[0], 0};
    Mat3x3d out;
    out.setZero();
    for (int i = 0; i < 9; i++) {
  ...
  void setConstraintWeight() override { constrainWeightSqrt = std::sqrt(area_rest * Triangle::k_stiff); };
