...
class Primitive {
public:
  enum PrimitiveType { PLANE, CUBE, SPHERE, CAPSULE, PINBALL, FOOT, LOWER_LEG, BOWL, DISK, TABLE };
  ...
  void updateForwardRecord() {
    ...
    if (isPrimitiveCollection) {
      for (Primitive *p : primitives) p->updateForwardRecord();
    }
  }
  ...
  virtual Vec3d getForces(double timeStep) {
    if (gravityEnabled) return gravity * mass;
    return Vec3d(0, 0, 0);
  }
  virtual void step(double timeStep) {
    if (!isStaitc) {
      if (gravityEnabled) velocity += gravity * timeStep;
  ...
  static Vec3d projectionOnPlane(Vec3d normal, double d, Vec3d p) { return (p - (normal.dot(p) + d) * normal); }
};
...
public:
  ...
  Mat3x3d rotMax =
      (Eigen::AngleAxis<double>(0, Vec3d::UnitX()) * Eigen::AngleAxis<double>(0.5 * 3.14159, Vec3d::UnitY()) *
          Eigen::AngleAxis<double>(0, Vec3d::UnitZ()))
          .toRotationMatrix();

  Sphere(Sphere &&other) : Primitive(std::move(other)) { radius = other.radius; }
  ...
  bool isInContact(const Vec3d &center_prim, const Vec3d &pos, const Vec3d &velocity, Vec3d &normal, double &dist,
...
public:
  ...
  Bowl(Bowl &&other) : Primitive(std::move(other)) { radius = other.radius; }
...
public:
  ...
  Vec3d axis, parentAxis,
      globalAxis;  // axis is relative to parentAxis,  globalAxis is the actual direciton of the joint
  ...
  Capsule(Vec3d anchorCenter, double radius, double length, Vec3d parentAxis, Vec3d axis, Capsule *parent = nullptr,
      bool isRoot = true, Primitive *rootPrimitiveContainer = nullptr,
      Vec3d myColor = Vec3d(0.671993, 0.395403, 0.0829298), int resSphere = 40, int resBody = 40);
  ...
  Vec3d getTransformedPosFromJointBindPos(Vec3d original) {
    ...
    accumCenter += joint->center;
    accumulated = accumulated * joint->rotationFromParent;
    if (joint->rootPrimitiveContainer != nullptr) accumCenter += joint->rootPrimitiveContainer->center;
